...
def min_distance_avoid(img, avoid_class, n_iters=15, steps=100, max_delta=10):
    """
    For 'not X' challenges: try every class except avoid_class,
    run min_budget_attack for each, and return the one with the lowest L1 distance.
    """
    best_x, best_dist = None, float('inf')
    for target in range(8):
        if target == avoid_class:
            continue
        candidate = min_budget_attack(img, target_class=target, n_iters=n_iters,
                                      steps=steps, max_delta=max_delta, min_confidence=0.0)
        with torch.no_grad():
...
easy_second = min_budget_attack(easy_first, target_class=1, min_confidence=0.0)
medium_second = min_budget_attack(medium_first, target_class=1, min_confidence=0.0)
hard_first = min_distance_avoid(hard_second, avoid_class=1)  # try all non-happy targets, pick lowest distance
